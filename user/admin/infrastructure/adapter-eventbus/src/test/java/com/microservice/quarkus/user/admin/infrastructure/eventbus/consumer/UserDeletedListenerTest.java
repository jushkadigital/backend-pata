...
class UserDeletedListenerTest {
    ...
    private UserDeletedListener createListener() {
        ...
        UserDeletedListener listener = new UserDeletedListener(adminService);
        listener.tracer = tracer;
        return listener;
    }

    @Test
    void onUserDeleted_shouldDeleteAdminByExternalId() {
        ...
        verify(adminService).deleteByExternalId(externalId);
    }
    ...
    void onUserDeleted_shouldPropagateExceptionOnFailure() {
        ...
        doThrow(new RuntimeException("DB down")).when(adminService).deleteByExternalId(anyString());
