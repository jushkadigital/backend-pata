...
class UserServiceTest {
    ...
    void register_shouldCreateNewUserWhenNotExists() {
        ...
        CreateUserCommand cmd = new CreateUserCommand("new@example.com", null, "pass123", "PASSENGER", "STANDARD", false);
        String result = createService().register(cmd);

        assertEquals("kc-123", result);
        verify(persistenceService).persistPendingSync(
            eq("new@example.com"), eq(UserType.PASSENGER), eq("kc-123"),
            any(), any(), any(), any());
    }
    ...
    void register_shouldReturnExistingExternalIdWhenAlreadySynced() {
        UserSyncRecord existing = new UserSyncRecord("id-1", "existing@example.com", "ext-123",
            UserType.PASSENGER, SyncStatus.SYNCED, null, null);
        when(syncRepository.findByEmail("existing@example.com")).thenReturn(existing);

        CreateUserCommand cmd = new CreateUserCommand("existing@example.com", null, "pass", "PASSENGER", "STANDARD", false);
        String result = createService().register(cmd);

        assertEquals("ext-123", result);
        verify(keycloakClient, never()).createUser(anyString(), anyString());
        verify(persistenceService, never()).persistSyncAndOutbox(any(), any(), any(), any(), any(), any(), any(), any());
    }
    ...
    void register_shouldHandleKeycloakFailure() {
        ...
        CreateUserCommand cmd = new CreateUserCommand("fail@example.com", null, "pass", "PASSENGER", "STANDARD", false);
        String result = createService().register(cmd);

        assertNull(result);
        verify(persistenceService).persistFailedSync("fail@example.com", UserType.PASSENGER);
        verify(persistenceService, never()).persistSyncAndOutbox(any(), any(), any(), any(), any(), any(), any(), any());
    }
    ...
    void register_shouldCreateUserCommandFromWebhook2() {
        CreateUserCommand cmd = CreateUserCommand.fromWebhook2(
                "user2@example.com", "password123", "PASSENGER", "PREMIUM");

        assertEquals("user2@example.com", cmd.email());
        assertEquals("password123", cmd.password());
        assertEquals("PASSENGER", cmd.type());
        assertEquals("PREMIUM", cmd.subtype());
    ...
    void register_shouldUseExternalIdWhenFromKeycloak() {
        ...
        CreateUserCommand cmd = CreateUserCommand.fromWebhook("webhook@example.com", "kc-webhook-123", "PASSENGER");
        String result = createService().register(cmd);

        assertEquals("kc-webhook-123", result);
        verify(keycloakClient, never()).createUser(anyString(), anyString());
        verify(persistenceService).persistSyncAndOutbox(
            eq("webhook@example.com"), eq(UserType.PASSENGER), eq("kc-webhook-123"),
            any(), any(), any(), any(), any());
    }
    ...
    void register_shouldNotCompensateWhenDbFails() {
        ...
        doThrow(new RuntimeException("DB connection lost"))
            .when(persistenceService).persistPendingSync(any(), any(), any(), any(), any(), any(), any());

        CreateUserCommand cmd = new CreateUserCommand("dbfail@example.com", null, "pass123", "PASSENGER", "STANDARD", false);

        assertThrows(RuntimeException.class, () -> createService().register(cmd));
        verify(keycloakClient, never()).deleteUser(anyString());
        verify(persistenceService, never()).persistFailedSync(any(), any());
    }
    ...
    void register_shouldSkipPendingRecordsAndReturnExternalId() {
        UserSyncRecord pendingRecord = new UserSyncRecord("id-2", "pending@example.com", "ext-pending",
            UserType.PASSENGER, SyncStatus.PENDING, null, null);
        when(syncRepository.findByEmail("pending@example.com")).thenReturn(pendingRecord);

        CreateUserCommand cmd = new CreateUserCommand("pending@example.com", null, "pass", "PASSENGER", "STANDARD", false);
        String result = createService().register(cmd);

        assertEquals("ext-pending", result);
        verify(keycloakClient, never()).createUser(anyString(), anyString());
        verify(persistenceService, never()).persistPendingSync(any(), any(), any(), any(), any(), any(), any());
    }
    ...
    void register_shouldReturnExistingExternalIdWhenAlreadyPending() {
        ...
        CreateUserCommand cmd = new CreateUserCommand("pending2@example.com", null, "pass", "PASSENGER", "STANDARD", false);
        String result = createService().register(cmd);
