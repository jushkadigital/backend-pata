...
public class UserCreatedListener {
  ...
  private final UserCreatedEventMapper eventMapper;
  ...
  @Inject
  public UserCreatedListener(PassengerService passengerService,
  ...
  @ConsumeEvent("identity.user.created.v1")
  @Blocking
  public void onUserCreated(String event) {

    UserCreatedEventDTO dto = new JsonObject(event).mapTo(UserCreatedEventDTO.class);
    ...
    if (parentCtx != null && parentCtx.isValid()) {
      span = tracer.spanBuilder("passenger.onUserCreated")
          .setParent(Context.root().with(Span.wrap(parentCtx)))
          .startSpan();
    } else {
      span = tracer.spanBuilder("passenger.onUserCreated").startSpan();
    }
    ...
    try (Scope scope = span.makeCurrent()) {
      ...
      if (!"PASSENGER".equals(dto.userType())) {
        log.debug("Passenger Module: Ignoring non-passenger event (userType={})", dto.userType());
        return;
      }

      if (!idempotencyStore.tryAcquire(dto.eventId().toString(), "passenger-inbound")) {
        log.info("Passenger Module: Idempotent skip — eventId={} already processed", dto.eventId());
        return;
      }

      log.info("Passenger Module: Received event for user {} userType={} clientRoles={}", dto.email(), dto.userType(), dto.clientRoles());
      ...
      try {
        ...
        log.error("Passenger Module: Failed to create passenger for aggregateId={}", dto.aggregateId(), e);
        throw e;
      }
